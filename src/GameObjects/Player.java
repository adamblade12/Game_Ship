...
public class Player extends MovingObject{
	...
	private long flickerTime,shieldTime, doubleScoreTime, fastFireTime, doubleGunTime;
	...
	public Player(Vector2D position, Vector2D velocity,double maxVel, BufferedImage texture, GameState gameState) {
		...
		soundShoot = new Sound(Assets.playerShoot);
	...
	public void update(float dt) {
		...
		if(spawning) {
			...
			if(spawnTime > Constants.SPAWN_TIME) {
				spawning = false;
				visible = true;
			}
		}
		...
		if(KeyBoard.SHOOT && fireRate > fireSpeed && !spawning) {
			...
			if(doubleGunOn) {
				Vector2D leftGun = center();
				Vector2D rightGun = center();
				...
				gameState.getMovingobjects().add(0,new Laser(
						center().add(heading.scale(width)),
						heading,
						Constants.LASER_VEL,
						angle,
						Assets.redLaser,
